...
void main() async {
  ...
  const apiBase = 'https://api.manejapp.app/api/v1';
  ...
  final userResponse = await HttpClient()
      .getUrl(Uri.parse('$apiBase/users/$userId'))
      .then((req) {
  ...
  final instructorsResponse =
      await HttpClient().getUrl(Uri.parse('$apiBase/instructors')).then((req) {
    req.headers.add('Authorization', 'Bearer $token');
    return req.close();
  }).then((res) => res.transform(utf8.decoder).join());
  ...
  final myInstructor =
      instructors.firstWhere((i) => i['id'].toString() == instructorId);
  ...
  final updateResponse = await HttpClient()
      .putUrl(Uri.parse('$apiBase/instructors/$instructorId'))
      .then((req) {
    req.headers.add('Authorization', 'Bearer $token');
  ...
  final verifyInstructor =
      verifyInstructors.firstWhere((i) => i['id'].toString() == instructorId);
  debugPrint(
      'Description después del update: ${verifyInstructor['description']}');
  debugPrint(
      '¿Se guardó? ${verifyInstructor['description'] == updateData['description']}');
}
