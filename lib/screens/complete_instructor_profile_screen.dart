...
/// La documentación no bloquea estos datos; `isListed` solo se envía si el backend marca `publishable`.
...
class CompleteInstructorProfileScreen extends StatefulWidget {
  ...
  @override
  State<CompleteInstructorProfileScreen> createState() => _CompleteInstructorProfileScreenState();
}

class _CompleteInstructorProfileScreenState extends State<CompleteInstructorProfileScreen> {
  ...
  Future<void> _loadInstructorSnapshot() async {
    try {
      ...
      setState(() {
        ...
        _userId = uid;
        if (me != null) {
          _publishable = me['publishable'] == true;
  ...
  Future<void> _pickAndUploadPhoto() async {
    if (_userId == null || _userId!.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('No encontramos tu usuario. Volvé a iniciar sesión.'),
  ...
  Future<void> _submit() async {
    ...
    if (expRaw.isNotEmpty) {
      ...
      if (exp == null || exp < 0 || exp > 50) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Años de experiencia: número entre 0 y 50'),
            backgroundColor: AppColors.error,
          ),
        );
        return;
    ...
    if (hourlyRaw.isNotEmpty) {
      ...
      if (hourly == null || hourly <= 0) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Ingresá un precio por hora mayor a 0'),
            backgroundColor: AppColors.error,
          ),
        );
        return;
    ...
    if (hasLocation) {
      ...
      if (addr.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Ingresá un nombre para tu ubicación en el mapa'),
            backgroundColor: AppColors.error,
          ),
        );
        return;
    ...
    if (body.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('No hay cambios para guardar. Completá al menos experiencia, descripción o tarifa, o elegí la zona en el mapa.'),
          backgroundColor: AppColors.error,
        ),
      );
      return;
    ...
    try {
      developer.log('[CompleteInstructorProfile] submit keys=${body.keys.toList()}', name: 'RoleRouter');
      ...
      if (Navigator.of(context).canPop()) {
        ...
        developer.log('[CompleteInstructorProfile] → $target', name: 'RoleRouter');
        if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Perfil profesional', style: AppTextStyles.heading.copyWith(fontSize: 20)),
        backgroundColor: AppColors.background,
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            AppInput(
                              ...
                              hint: 'Contá tu experiencia y estilo de enseñanza',
                        ...
                        child: AppCard(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              AppButton(
                                text: _uploadingPhoto ? 'Subiendo…' : 'Elegir foto de perfil',
                          ...
                          children: [
                            ...
                            AppCard(
                              ...
                              child: ClipRRect(
                                ...
                                child: SizedBox(
                                  height: ResponsiveLayout.embeddedMapHeight(context),
                        ...
                        child: _publishable
                            ? AppCard(
                                ...
                                child: SwitchListTile(
                                  ...
                                  onChanged: _isLoading ? null : (v) => setState(() => _isListed = v),
                                  title: Text(
                            ...
                            : AppCard(
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    ...
                                    Expanded(
                                      child: Text(
                                        'Cuando la documentación esté aprobada y tu perfil sea publicable, vas a poder activar la visibilidad desde acá.',
                                        style: AppTextStyles.bodyNormal.copyWith(fontSize: 13, height: 1.4),
                                      ),
                                    ),
                                  ],
                                ),
