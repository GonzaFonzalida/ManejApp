...
class _AdminUsersScreenState extends State<AdminUsersScreen>
  ...
  Widget _buildUsersList(List<dynamic> users) {
    if (users.isEmpty) {
      return const Center(
          child: Text('No hay usuarios', style: TextStyle(color: Colors.grey)));
    }
        ...
        itemBuilder: (context, index) {
          ...
          return Card(
            ...
            child: ListTile(
              ...
              title: Text(
                  '${user['user']?['firstName'] ?? user['firstName'] ?? user['name'] ?? ''} ${user['user']?['lastName'] ?? user['lastName'] ?? user['surname'] ?? ''}'),
              subtitle: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(user['user']?['email'] ?? user['email'] ?? ''),
                  Text('Rol: ${user['user']?['role'] ?? user['role'] ?? 'N/A'}',
              ...
              trailing: Row(
                ...
                children: [
                  ...
                  PopupMenuButton(
                    itemBuilder: (context) => [
                      const PopupMenuItem(
                          value: 'view', child: Text('Ver detalles')),
                    ],
  ...
  Future<void> _toggleUserStatus(dynamic user, bool isActive) async {
    final userId =
        (user['user']?['id'] ?? user['userId'] ?? user['id']).toString();
    ...
    try {
      debugPrint(
          'Toggling user $userId to ${isActive ? "active" : "inactive"}');
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            _buildDetailRow(
                'Correo', user['user']?['email'] ?? user['email'] ?? 'N/A'),
            _buildDetailRow(
                'DNI', user['user']?['dni'] ?? user['dni'] ?? 'N/A'),
            _buildDetailRow(
                'Rol', user['user']?['role'] ?? user['role'] ?? 'N/A'),
            _buildDetailRow('Fecha de nacimiento',
                user['user']?['birthDate'] ?? user['birthDate'] ?? 'N/A'),
          ],
        ),
        actions: [
          TextButton(
